...
```{block, type='tip'}
There is a `sklearn.preprocessing.power_transform` function that can be applied to immediately transform an array. However, later in this module we'll discuss the idea of data leakage and how important it is to create isolated preprocessing steps.
```
...
In R we use `recipes::recipe()` to build a "recipe" of preprocessing steps. This will not return the actual log transformed values but, rather, a blueprint to be applied later.
...
rare_encoder = RareLabelCategoricalEncoder(tol=0.01, replace_with="other")
```
...
```{r engineering-minimize-leakage, echo=FALSE, fig.cap="__Figure__: Performing feature engineering preprocessing within each resample helps to minimize data leakage.", out.width='90%'}
...
We can now create a modeling pipeline that includes the preprocessing step and then the modeling step. Similar to `ColumnTransformer`, `Pipeline` takes tuples with the name and object for each step in the order you want them applied.
...
4. Compare the Python and R preprocessing steps in the last section. Can you spot any discrepencies? If so, resolve them so the pipelines are performing the same steps.
