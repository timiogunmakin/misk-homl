...
First, unlike random forests in R, XGBoost in R requires all features to be numeric. Consequently, we need to ordinal encode our Quality/Condition features (i.e. `Overall_Qual`, `Garage_Qual`, `Kitchen_Qual`) and, as usual, we need to one-hot encode our remaining nominal features. The following recipe follows similar procedures we applied in the feature engineering module.
...
Next, we use `boost_tree()` with the `xgboost` engine to apply a default XGBoost model.
...
Due to the many hyperparameters in GBMs it is far more efficient to use a random grid search rather than a full cartesian search. For brevity, in the following example we search across 20 random combinations of number of `trees`, learning rate (`learn_rate`), max depth (`tree_depth`), and minimum observations required in nodes (`min_n`). 

Here, we use `parameters()` to extract the hyperparameters we plan to tune in `xgb_mod` and then we use `grid_latin_hypercube` to create our random design.

```{block, type='tip'}
Read more about working with tidymodel parameters and creating different grid search designs [here](https://www.tmwr.org/grid-search.html#irregular-grids).
```
...
Now we can perform our grid search. We see that the optimal model of the random search had an RMSE of 26752 and used 81 trees, `min_n` of 23, tree depth of 9, and a learning rate of 0.0683. 

```{block, type='warning'}
This grid search takes 40 minutes!
```

```{block, type='tip'}
At this point, it would probably be wise to follow up with another search that uses the `learning_rate`, `tree_depth`, and `min_n` of the optimal model but search across additional trees to see if adding more trees improves performance. For brevity we'll leave this as an exercise for the reader.
```
...
## 1    81    23          9    0.0683  rmse    standard   26752.     5   2856.
## 2   529    24          7    0.0263  rmse    standard   26923.     5   2849.
## 3  1076    37          2    0.00847 rmse    standard   27461.     5   2667.
## 4  1718     6          6    0.00110 rmse    standard   43209.     5   2171.
## 5   774    31          2    0.00172 rmse    standard   65370.     5   2610.
...
The following takes the best basic GBM model found in the previous section and builds upon it by adding stochastic components. We perform another grid search across the various stochastic parameters available in XGBoost via the Python and R APIs.

### `r fontawesome::fa("python")` 

In the Python API we will assess the following stochastic parameters: (1) `subsample` rows before each tree and subsampling columns before each tree (`colsample_bytree`), level (`colsample_bylevel`), and split (`colsample_bynode`).
...
In the R API we will assess the following stochastic parameters: (1) subsample rows before each tree (`sample_size`) and (2) subsampling columns before each  split (`mtry`). 

```{block, type='tip'}
Similar to what we did in the random forest module, we can pass additional hyperparameters specific to XGBoost by including them in `set_engine()` call.
```
...
```{block, type='warning'}
This grid search takes only 2 minutes!
```
...
## 1    66       0.828 rmse    standard   25761.     5   2673. Preproces…
## 2    49       0.698 rmse    standard   25767.     5   2769. Preproces…
## 3    65       0.751 rmse    standard   25812.     5   2813. Preproces…
## 4    78       0.968 rmse    standard   25906.     5   2807. Preproces…
## 5    31       0.778 rmse    standard   26009.     5   2496. Preproces…
```
...
The following only applies the gamma hyperparameter via `boost_tree`'s `loss_reduction` parameter. Similar to what we did in the random forest module, we could perform a grid search across XGBoost's `alpha` and `lambda` regularization parameters by passing them in the `set_engine()` call; however, we'll leave that as a task for the reader.

Similar to the Python example we do not see much improvement by adding regularization.
...
## 1       4.64e- 3 rmse    standard   25568.     5   2681. Preprocessor…
## 2       1   e-10 rmse    standard   25679.     5   2750. Preprocessor…
## 3       3.16e+ 1 rmse    standard   25710.     5   2666. Preprocessor…
## 4       1.90e- 9 rmse    standard   25787.     5   2674. Preprocessor…
## 5       3.59e- 8 rmse    standard   25872.     5   2749. Preprocessor…
```
...
In this example we see that we actually get the best performance with a lower number of trees. We could actually perform another search that assesses even less number of trees, which could provide compute efficiency. However, we'll leave that as an exercise for the reader.
...
In the R API we can apply `stop_iter` within our `boost_tree()` call. This allows us to crank up the number of trees to apply but automatically stop if we don't experience some improvement in the loss for `N` iterations.

In this example we see that we actually get some improvement in our loss by using more trees.
...
## 1 rmse    standard   24674.        5 2130.     Preprocessor1_Model1
## 2 rsq     standard       0.906     5    0.0195 Preprocessor1_Model1
```
